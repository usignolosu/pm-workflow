...
> **v7.0.0 合并说明**：本技能已合并姊妹工作流仓库（已归档）的工程化资产——契约校验、状态机、知识图谱、18 个脚本。
...
- `true`：⑦ 之后继续出代码 + 测试报告（对应原 姊妹工作流仓库的 standard/complex 完整链路）
- `false`：只出文档 + 原型，不写代码（对应原 姊妹工作流仓库的 `lean_pm` 模式）
...
- `false`（原 姊妹工作流仓库 `lean_pm`）：止于 ⑦，只交付文档 + 原型
...
> 手写原型常用 `go()`、也有原型用 `select()`、`gen_prototype.py` 产出用 `show()`。
...
├── 归档需求产出/<REQ-ID>/              # ★ 只读历史归档（姊妹工作流仓库迁入 REQ-001/002/004/005，不再写入）
├── contracts/                          # 契约（宪法/guardrails/spec_checklist）
└── knowledge_graph/                    # 知识图谱
```

> **范式说明（Route X · 弃用 A 切 B）**：
> - `归档需求产出/` 仅为**只读历史档案**，保留 姊妹工作流仓库并入时的原 REQ 编号，活动工作流永不写入；仅 `validate_contract.py`、`ingest_to_kg.py` 读取它做契约校验与 KG 入库。
...
> 2. `<需求名>需求产出/` —— **交付归口层**：面向人/评审的最终产物落盘处（PRD、原型、截图、用例），命名统一为「<项目名>需求产出/」形式。
> 3. `归档需求产出/<REQ-ID>/` —— **只读历史层**：仅 姊妹工作流仓库并入的 REQ-001/002/004/005，永不写入。
> - 同一需求的 `runs/<run-id>/` 与 `<需求名>需求产出/` 是**同一份交付的两个视图**（过程 vs 归口），非冗余副本；历史 `归档需求产出/` 不在该链条内。
> - 现有交付目录均已符合本约定（统一为「<项目名>需求产出/」形式），无需再做统一。
